...
parser.add_argument('--batch-size', type=int, help='Batch size for the model during training.', default=1)
...
counter = -1
img_num = 0
for img, mask in tqdm(test_gen, total=test_len//args.batch_size):
    all_pred = model.predict(img)
    for i in range(len(all_pred)):
        ...
        if args.save_masks:
            ...
            im_name = f'pred_{img_num}.jpg'
            im.save(os.path.join('preds', im_name))
    
        im_gt = np.argmax(mask, axis=-1)
        
        acc = accuracy_multilabel(im_gt, im_pred, mask.shape[-1])
        dice = dice_coef_multilabel(im_gt, im_pred, mask.shape[-1])
        jacc = jaccard_coef_multilabel(im_gt, im_pred, mask.shape[-1])
        tfstats = tf_stats_multiclass(im_gt, im_pred, mask.shape[-1])
