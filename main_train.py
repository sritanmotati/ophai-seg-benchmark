...
parser.add_argument('--binary', type=int, help='Whether the segmentation masks are binary (1) or multi-class (0).', default=0)
...
history = model.train(train_gen, val_gen, train_len//args.batch_size, val_len//args.batch_size, osp.join(sp,f'{args.model_name}_best' + ['.h5', '.pth'][args.model_name in torch_models]))
model.save(osp.join(sp,f'{args.model_name}_final' + ['.h5', '.pth'][args.model_name in torch_models]))
